...
class OpeningViewModel : ViewModel() {
    ...
    fun importStagesUserData(user: FirebaseUser) {
        val stages = mutableListOf<com.fasila.aqsalandmarks.model.realtime.Stage>()

        val stagesListener = object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                for (stageSnapshot in dataSnapshot.children) {
                    if (stageSnapshot.getValue<com.fasila.aqsalandmarks.model.realtime.Stage>() != null) {
                        stages.add(stageSnapshot.getValue<com.fasila.aqsalandmarks.model.realtime.Stage>() as com.fasila.aqsalandmarks.model.realtime.Stage)
    ...
    fun importBadgesUserData(user: FirebaseUser) {
        val badges = mutableListOf<com.fasila.aqsalandmarks.model.realtime.Badge>()

        val badgesListener = object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                for (badgeSnapshot in dataSnapshot.children) {
                    if (badgeSnapshot.getValue<com.fasila.aqsalandmarks.model.realtime.Badge>() != null) {
                        badges.add(badgeSnapshot.getValue<com.fasila.aqsalandmarks.model.realtime.Badge>() as com.fasila.aqsalandmarks.model.realtime.Badge)
